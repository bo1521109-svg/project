...
// TikTok店铺爆卖榜（销量榜）API - 后端prefix: /tiktok/stores/best-sellers
const API_PATH = '/v1/tiktok/stores/best-sellers'

// 接收筛选组件发出的搜索参数，调用后端API获取店铺爆卖榜数据
const handleFilterSearch = async (params) => {
  try {
    const response = await request.get(API_PATH, { params })
    console.log('店铺爆卖榜数据:', response.data)
  } catch (e) {
    console.error('获取店铺爆卖榜失败:', e)
  }
}
</script>
