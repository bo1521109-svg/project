<template>
  <div class="tk-product-overview-page">
...
<script setup>
/*
 * TikTok商品大盘 - 从Shopify商品大盘复制并适配
 * 差异：API路径使用TikTok前缀，三级点击跳转使用TikTok商品搜索页
 */
...
const fetchLevel1 = async () => {
  try {
    const res = await request.get('/v1/tiktok/product/overview/level1')
...
const fetchLevel2 = async (code, name) => {
  try {
    const res = await request.get('/v1/tiktok/product/overview/level2', {
...
const fetchLevel3 = async (l2Code, l2Name) => {
  try {
    const res = await request.get('/v1/tiktok/product/overview/level3', {
