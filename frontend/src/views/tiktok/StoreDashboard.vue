<template>
  <div class="tk-store-dashboard-page">
...
<script setup>
/*
 * TikTok店铺大盘 - 从Shopify店铺大盘复制并适配
 * 差异：API路径使用TikTok前缀，类目使用TikTok类目
 */
...
const categoryOptions = computed(() => {
  const topLevel = tiktokCategories.filter(c => c.value !== 'all')
  return [
    { label: '全部', value: 'all' },
...
const fetchData = async () => {
  try {
    const [metricsRes, top10Res] = await Promise.all([
      request.get('/v1/tiktok/store/overview/metrics', {
        params: {
      ...
      request.get('/v1/tiktok/store/overview/top10', {
        params: {
