<template>
  <div class="tk-category-overview">
    <div class="top-bar">
      <el-input
...
<script setup>
/*
 * TikTok品类大盘 - 从Shopify品类大盘复制并适配
 * 差异：API路径使用TikTok前缀，CategoryBlock使用TikTok版本（跳转到TikTok商品搜索）
 */
...
import CategoryBlock from '@/components/TikTok/CategoryBlock.vue'
...
const cardSpanMap = computed(() => {
  ...
  for (const col of colCats) {
    ...
    let spans = salesList.map(s => Math.max(2, Math.round((s / colTotal) * maxRows)))
    let sum = spans.reduce((s, v) => s + v, 0)
    if (sum > maxRows) {
      const factor = maxRows / sum
      spans = spans.map(s => Math.max(2, Math.round(s * factor)))
      sum = spans.reduce((s, v) => s + v, 0)
    }
    while (sum > maxRows) {
      const maxIdx = spans.indexOf(Math.max(...spans))
      if (spans[maxIdx] > 2) { spans[maxIdx]--; sum-- } else break
    }
    while (sum < maxRows) {
      const maxIdx = spans.indexOf(Math.max(...spans))
      spans[maxIdx]++; sum++
...
const getCardSpan = (cat) => {
  return cardSpanMap.value[cat.name] || 2
}
...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await request.get('/v1/tiktok/category/overview', { params })
